...
| 仕上げ | 6-1 | 5分毎Cronへの接続（Gmail取得→AI整理→通知の一連処理を自動実行） | 未着手 |
...
- **要対応**: CRON_EVERY_5_MIN（5分毎Cron）は現状ログ出力のみで、`/debug/gmail-check`相当の一連処理（Gmail取得→AI整理→通知）にまだ接続されていない。6-1（本番デプロイ）の前に、この接続作業を別スプリントとして追加する必要がある
