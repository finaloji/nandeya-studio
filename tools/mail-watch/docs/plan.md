...
| AI | 2-2 | 通知要否の最終判定（rep かつ 緊急度low以外のみLINE通知対象） | 完了 |
| 通知 | 3-1 | LINE push通知（Flex Message・Gmailリンク・管理画面リンクは未実装のため保留） | 完了 |
| 通知 | 3-2 | 通知履歴の記録（action_logs・notify_count） | 完了 |
| 管理画面 | 4-1 | メール一覧（ステータス別タブ）※パスコード認証は当面なし | 完了 |
| 管理画面 | 4-2 | ステータス更新（確認済み/対応中/完了）＋操作履歴表示 | 完了 |
