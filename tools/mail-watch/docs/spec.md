...
> 作成日: 2026-07-17
> ステータス: 本番稼働中（2026-07-19デプロイ）
> 公開URL: https://mail-watch.nandeya-ai.workers.dev/（管理画面）
...
[Web管理画面（HTTP）]
  メール一覧（ステータス別）※パスコード認証は当面なし（2026-07-19決定・後日追加）
...
- パスコード認証は**当面実装しない**（2026-07-19決定。URLを知っていれば誰でも閲覧・操作できる状態で運用する。後日必要になれば追加する）
...
| ADMIN_PASSCODE_HASH | 管理画面パスコードの SHA-256 ハッシュ（当面認証を実装しないため未使用。将来の追加用に環境変数の枠だけ残す） |
...
- [ ] 朝のまとめ通知の時刻（仮: 8時）
- [x] Gemini の使用モデル — `gemini-flash-latest`（2026-07-17決定。`gemini-2.0-flash`は無料枠クォータが0で使えなかったため、常に最新のFlashを指すエイリアスを採用）
